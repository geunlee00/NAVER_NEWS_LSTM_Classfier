...
def train_model(config: Config) -> Tuple[TextLSTMClassifier, Dict[str, object]]:
    ...
    report = classification_report(all_targets, all_preds, target_names=[id_to_label[i] for i in range(len(id_to_label))], zero_division=0, output_dict=True)
    metadata = {"vocab": vocab, "label_to_id": label_to_id, "id_to_label": id_to_label, "accuracy": accuracy, "epoch_losses": epoch_losses, "report": report} # 호출자에게 반환할 메타데이터를 구성한다.
